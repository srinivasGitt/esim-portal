...
export class RevenueComponent implements OnInit {
  ...
  getReports(value?: any, fromDate?: any, toDate?: any) {
    ...
    this.dashboardService.getReports(value, fromDate, toDate).subscribe(
      (res: any) => {
        if (res.result) {
          const labelData: any[] = [];
  ...
  downloadReport() {
    ...
    this.reportService
      .getDownloadReport(timeFrame, this.startDate, this.endDate)
      .subscribe((res: any) => {
        if (res && res.length <= 0) {
  ...
  sendTransactionAndRevenueReportMail() {
    ...
    this.reportService
      .sendTransactionAndRevenueReportMail(timeFrame, this.startDate, this.endDate)
      .subscribe(
        (res: any) => {
          this.dialogService
            .openModal(ReportSuccessInfoComponent, {
