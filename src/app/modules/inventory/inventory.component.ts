...
export class InventoryComponent implements OnInit, OnDestroy {
  ...
  downloadModal() {
    this.dialogService
      .openModal(DownloadSampleFileComponent, {
        cssClass: 'modal-sm',
        context: { data: {}, title: 'Download Sample File' },
      })
      .instance.close.subscribe((data: any) => {
        if (data) {
        }
  ...
  showInventory(inventory: any) {
    this.dialogService
      .openModal(InventoryInfoComponent, { cssClass: 'modal-sm', context: { data: inventory } })
  ...
  getPageNumber(event: any) {
    ...
    if (this.inSearch && this._searchService.searchedTerm.length > 3) {
      ...
      this.inventoryService
        .listInventory(this.paginateConfig.itemsPerPage, this.paginateConfig.currentPage - 1)
        .subscribe(
          (res: any) => {
            this.inventories = res.data;
            this.paginateConfig.totalItems = res?.count[0]?.totalCount;
