...
export class SubscribeManagementComponent implements OnInit, OnDestroy {
  ...
  createSubscriber() {
    this.dialogService
      .openModal(SubscriberMgmtComponent, {
        cssClass: 'modal-md',
        context: { data: {}, title: 'Add New Subscriber' },
      })
  ...
  editSubscriber(index: number) {
    this.dialogService
      .openModal(SubscriberMgmtComponent, {
        cssClass: 'modal-md',
        context: { data: this.subscriberList[index], title: 'Edit Subscriber' },
      })
      .instance.close.subscribe((data: any) => {
        if (data) {
          const vm = this;
  ...
  deleteSubscriber(subscriber: any) {
    ...
    this.dialogService
      ...
      .instance.close.subscribe((data: any) => {
        const vm = this;
        if (data) {
          vm.subscriberService.deleteSubscriber(subscriber._id).subscribe(
            (res: any) => {
  ...
  getPageNumber(event: any) {
    ...
    if (this.inSearch && this._searchService.searchedTerm.length > 3) {
      ...
      this.subscriberService
        .getAllSubscriber(this.paginateConfig.itemsPerPage, this.paginateConfig.currentPage - 1)
        .subscribe(
          (res: any) => {
            this.subscriberList = res.data;
            this.paginateConfig.totalItems = res?.count[0]?.totalCount;
